    ...
    function log(){
        var start = Date.parse(document.getElementById('start').value) / 1000;
        var end = Date.parse(document.getElementById('end').value) / 1000;

        setTimeout(invokeEventLogAPI(start, end),500);
        setTimeout(invokeLocLogAPI(start, end),500);
    }

    function invokeEventLogAPI(start, end) {
        var API_URI = '/work?type=scan&start='+start+'&end='+end;
        ...
        $.ajax('api' + API_URI, {
            ...
            success: function (data, status, xhr) { 
               printEventLogList(data,'#event_log');
            },
    ...
    // 데이터 출력을 위한 함수
    function printEventLogList(data, place){
        $(place).empty();  
        if (data.length > 0) {
